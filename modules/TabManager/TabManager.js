...
class TabManager extends EventEmitter {
    ...
    newTab() {
        // Default new tab opens the overlay page
        const url = require("url").pathToFileURL(require("path").join(this.appPath, "html", "overlay.html")).toString();
    ...
    newBackgroundTab() {
        // Default background tab opens the overlay page
        const url = require("url").pathToFileURL(require("path").join(this.appPath, "html", "overlay.html")).toString();
    ...
    const isInternal = (u) => {
        ...
        return clean.endsWith("/html/overlay.html") || clean.endsWith("/html/settings.html") || clean.endsWith("/html/about.html");
    };
    let tab = new Tab(this.window, id, this.appPath, this.tabGroup, this.theme, { nodeIntegration: isInternal(url) });
    ...
    this.window.webContents.send("tabRenderer-addTab", { id, url, active });
        ...
        // Notify main to register overlay tab webContents when creating overlay tab
        try {
            if (typeof url === "string") {
                const clean = url.split("?")[0].split("#")[0];
                if (clean.endsWith("/html/overlay.html")) {
                    this.emit("overlay-tab-created", tab);
                }
            }
        } catch (e) {}
