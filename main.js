...
app.on("ready", () => {
  app.setUserTasks([{
      ...
      iconIndex: 0,
  title: "Show overlay",
  description: "Open the overlay page"
    }
  ]);
...
ipcMain.on("main-bookmarkAllTabs", (event) => {
  ...
  if(!sendToOverlayChannel("bookmarkManager-addFolderWithBookmarks", "Bookmarked tabs", arr)) {
...
ipcMain.on("main-updateTheme", (event) => {
  ...
  if(!sendToOverlayChannel("overlay-updateTheme")) {
    overlay.updateTheme();
  }
});
...
ipcMain.on('request-side-menu', (event, arg) => {
  Menu.buildFromTemplate(sideMenuConstructor(tabManager.hasActiveTab(), tabManager.hasTabs())).popup(mainWindow);
});
...
ipcMain.on("overlay-showButtonMenu", (event) => {
  // Button removed; show overlay tab instead
  openOrActivateOverlayTab();
});

ipcMain.on("overlay-goToSearch", (event, text, cursorPos) => {
  if(!sendToOverlayChannel("searchManager-goToSearch", text, cursorPos)) {
...
ipcMain.on("overlay-clearDownloads", (event) => {
  if(!sendToOverlayChannel("downloadManager-clearDownloads")) {
    overlay.clearDownloads();
  }
});

ipcMain.on("overlay-removeFolder", (event, id) => {
  if(!sendToOverlayChannel("bookmarkManager-removeFolder", id)) {
...
ipcMain.on("overlay-bookmarkThisPage", (event) => {
  if(tabManager.hasActiveTab()) {
    ...
    if(!sendToOverlayChannel("bookmarkManager-addBookmark", at.getTitle(), at.getURL())) {
...
ipcMain.on("overlay-setSearchEngine", (event, engine) => {
  if(!sendToOverlayChannel("searchManager-setSearchEngine", engine)) {
    overlay.setSearchEngine(engine);
  }
});

ipcMain.on("overlay-checkIfBookmarked", (event, url) => {
  if(!sendToOverlayChannel("bookmarkManager-checkIfBookmarked", url)) {
    overlay.checkIfBookmarked(url);
  }
});

ipcMain.on("overlay-showBookmarkOptions", (event, id) => {
  if(!sendToOverlayChannel("bookmarkManager-showBookmarkOptions", id)) {
...
// Open or focus the Settings page as a tab, optionally with a category
function openOrActivateSettingsTab(categoryId) {
  try {
    const settingsPath = path.join(app.getAppPath(), "html", "settings.html");
    const overlaySuffix = "/html/settings.html";
    const fullUrl = pathToFileURL(settingsPath).toString();
    const tabs = tabManager.getTabs();
    for (let i = 0; i < tabs.length; i++) {
      const t = tabs[i];
      let url = "";
      try { url = t.getURL(); } catch (e) { url = ""; }
      if (url && url.endsWith(overlaySuffix)) {
        t.activate();
        if (categoryId) {
          t.getWebContents().send("settings-showCategory", categoryId);
        }
        return;
      }
    }
    tabManager.addTab(fullUrl, true);
    // category will be sent on first dom-ready via a short timeout
    setTimeout(() => {
      if (tabManager.hasActiveTab()) {
        try { tabManager.getActiveTab().getWebContents().send("settings-showCategory", categoryId); } catch(e) {}
      }
    }, 300);
  } catch (e) { }
}

// Open or focus the About page as a tab
function openOrActivateAboutTab() {
  try {
    const aboutPath = path.join(app.getAppPath(), "html", "about.html");
    const suffix = "/html/about.html";
    const fullUrl = pathToFileURL(aboutPath).toString();
    const tabs = tabManager.getTabs();
    for (let i = 0; i < tabs.length; i++) {
      const t = tabs[i];
      let url = "";
...
ipcMain.on("tabManager-closeActiveTab", () => {
  if (tabManager && tabManager.hasActiveTab()) {
...
ipcMain.on("tabManager-findInPage", (event, text, forward) => {
  if(tabManager.hasActiveTab()) {
    tabManager.getActiveTab().findInPage(text, forward);
  }
});

ipcMain.on("tabManager-stopFindInPage", (event, keepSelection) => {
  if(tabManager.hasActiveTab()) {
    tabManager.getActiveTab().stopFindInPage(keepSelection);
  }
});
...
function initTabManager(theme) {
  ...
  tabManager.on("add-history-item", (url) => {
    if(!sendToOverlayChannel("historyManager-insertBeforeHistoryItem", url)) {
  ...
  tabManager.on("bookmark-tab", (title, url) => {
    if(tabManager.hasActiveTab()) {
      if(!sendToOverlayChannel("bookmarkManager-addBookmark", title, url)) {
  ...
  tabManager.on("open-history", (text) => {
    if(!sendToOverlayChannel("overlay-scrollToId", "history-title")) {
      overlay.scrollToId("history-title");
    }
  });

  tabManager.on("tab-group-switched", (tabGroup) => {
    if(!sendToOverlayChannel("overlay-switchTabGroup", tabGroup)) {
      overlay.switchTabGroup(tabGroup);
    }
    openOrActivateOverlayTab();
  });

  // Track overlay tab webContents for routing overlay IPC
  tabManager.on("overlay-tab-created", (tab) => {
    try {
...
// Open or focus the overlay page as a regular tab
function openOrActivateOverlayTab() {
  try {
  const overlaySuffix = "/html/overlay.html";
    const tabs = tabManager.getTabs();
    for (let i = 0; i < tabs.length; i++) {
      const t = tabs[i];
      let url = "";
      try { url = t.getURL(); } catch (e) { url = ""; }
      if (url && url.endsWith(overlaySuffix)) {
        overlayTabWebContents = t.getWebContents();
        t.activate();
        mainWindow.webContents.send("overlay-toggleButton", true);
        return;
      }
    }
  const overlayUrl = pathToFileURL(path.join(app.getAppPath(), "html", "overlay.html")).toString();
  tabManager.addTab(overlayUrl, true);
    // overlayTabWebContents will be set on did-navigate of the new tab below
  } catch (e) {
...
function sendToOverlayChannel(channel, ...args) {
  if (overlayTabWebContents && !overlayTabWebContents.isDestroyed()) {
    overlayTabWebContents.send(channel, ...args);
    return true;
  }
  return false;
}
